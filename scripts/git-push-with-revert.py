...
try:
    ...
    print(f"[v0] Commit hash: {commit_hash}")

    # Save commit info for potential revert
    revert_info = {
        'timestamp': datetime.now().isoformat(),
        'commit_hash': commit_hash,
        'branch': 'v0/ddiddimmo-4985-db65374e',
        'message': 'Admin redesign — role-based layouts',
        'files_changed': ['admin.html', 'admin-init.js', 'styles.css', 'ADMIN_REDESIGN.md']
    }
    
    with open('.revert_info.json', 'w') as f:
        json.dump(revert_info, f, indent=2)
    print("[v0] ✓ Saved revert info to .revert_info.json")

    # Push to remote
    subprocess.run(['git', 'push', 'origin', 'HEAD'], check=True)
    print("[v0] ✓ Pushed to remote successfully")
    
    print(f"\n[v0] To REVERT these changes later, run:")
    print(f"[v0]   git revert {commit_hash}")
    print(f"[v0]   git push origin HEAD")
